...
// Apply merge tag replacement to all boilerplate content
...
function el_render_engagement_letter_html( array $pdf_data ): string {
    // Get form data for merge tags
    $form_data = $pdf_data['form_data'] ?? [];
    
    // Load ACF boilerplate like the template does
    $letterhead_raw        = function_exists('get_field') ? (get_field('boilerplate_letterhead', 'option') ?: '<h1>Studio Legale Metta</h1>') : '';
    ...
    ob_start();
    // variables available to the partial:
    include get_stylesheet_directory() . '/template-parts/el/preview-inline.php';
    return ob_get_clean();
}
