...
/**
 * Formats full address from form data array
 * 
 * @param array $data Form data with address fields
 * @return string Formatted multi-line address
 */
function el_format_full_address($data) {
    if (empty($data)) {
        return '';
    }
...
/**
 * Retrieves all products in current cart
 * 
 * @return array Array of product IDs
 */
function el_get_cart_product_ids() {
    if (!el_ensure_cart()) {
        return [];
    }
 ...
 */
function el_is_product_in_cart($product_id) {
    return in_array($product_id, el_get_cart_product_ids());
}

/**
 * Retrieves cart item by product ID
 * 
 * @param int $product_id Product ID
 * @return array|null Cart item data or null if not found
 */
function el_get_cart_item_by_product_id($product_id) {
    if (!el_ensure_cart()) {
        return null;
    }
...
/**
 * Counts items in cart (excluding grouped parents if removed)
 * 
 * @return int Cart item count
 */
function el_get_cart_count() {
    if (!el_ensure_cart()) {
        return 0;
    }
